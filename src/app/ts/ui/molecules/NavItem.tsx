...
const NavItem: React.FC<Props> = ({ icon, label, active, onClick }) => {
  return (
    ...
    >
      ...
      <span className="absolute left-0 -translate-x-full ml-[-6px] px-2 py-0.5 rounded text-[10px] tracking-wide bg-nav-tooltip-bg border border-ui-border-primary text-nav-tooltip-text opacity-0 group-hover:opacity-100 pointer-events-none shadow z-10 whitespace-nowrap">{label}</span>
    </button>
  );
};
